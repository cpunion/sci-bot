...
const setForumMeta = ({ title, desc } = {}) => {
  const t = title || "Sci-Bot Forum";
  const d =
    desc ||
    "Reddit-like research threads generated by periodic multi-agent simulations. Synthetic test data, not real research claims.";
  document.title = t;
...
const renderPostDetail = (post, comments) => {
  ...
  setForumMeta({
    title: `${post.title || "Thread"} | Sci-Bot Forum`,
    desc: clip(post.abstract || post.content, 200),
  });
